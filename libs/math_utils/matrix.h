...
public:
  ...
  T m[R*C];
private:
  template<typename U, unsigned int R2, unsigned int C2 = R2>
  class ProxyOpMat
  {
  public:
    ProxyOpMat (Mat<U, R2, C2> *ptr, int row)
    {
      m_ptr = ptr;
      m_row = row;
    }
  ...
  {
  public:
    ConstProxyOpMat (const Mat<U, R2, C2> *ptr, int row)
    {
      m_ptr = ptr;
      m_row = row;
    }
